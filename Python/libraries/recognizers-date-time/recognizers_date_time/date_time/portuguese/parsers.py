...
class PortugueseTimeParser(BaseTimeParser):
    ...
    def parse_specific_time_of_day(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        start_index = len(PortugueseDateTime.Tomorrow) if trimmed_text.startswith(PortugueseDateTime.Tomorrow) else 0
        ...
        matches = list(
            RegExpUtility.get_safe_reg_exp(PortugueseDateTime.TimeOfDayRegex).finditer(trimmed_text[start_index:])
        )
        if matches:
            match = matches[0]
