...
class BaseDateTimePeriodExtractor(DateTimeExtractor):
    ...
    def merge_date_with_time_period_suffix(self, text: str, date_ers: [ExtractResult], time_ers: [ExtractResult]):
        ...
        while i < len(extracted_results) - 1:
            ...
            if (
                extracted_results[i].type == Constants.SYS_DATETIME_DATE
                and extracted_results[j].type == Constants.SYS_DATETIME_TIME
            ):
            ...
            if match:
                ...
                if not after_str[0 : match.start()] or after_str[0 : match.start()].isspace():
                    start = extracted_result.start
                        ...
                        extracted_result.start
                        + extracted_result.length
                        + len(RegExpUtility.get_group(match, Constants.TIME_OF_DAY_GROUP_NAME))
                ...
                if RegExpUtility.is_exact_match(self.config.middle_pause_regex, connector_str, False):
                    ...
                    if ending_match:
                        tokens.append(
                            Token(
                                extracted_result.start, extracted_result.start + extracted_result.length + match.end()
                            )
                        )
                        ...
                        for time_period in time_extract_results:
                            ...
                            if (not mid_str or mid_str.isspace()) and not time_period.meta_data:
                                tokens.append(
                                    Token(
                                        time_period.start,
                                        time_period.start + time_period.length + len(mid_str) + token.length,
                                    )
                                )
    ...
    def match_simple_cases(self, source: str, reference: datetime) -> List[Token]:
        ...
        simple_cases_matches = list(
            filter(lambda t: t != [], map(lambda x: list(regex.finditer(x, source)), self.config.simple_cases_regexes))
        )
            ...
            for match in matches:
                ...
                if before_str:
                    ...
                    if er:
                        ...
                        if not middle_str or RegExpUtility.is_exact_match(
                            self.config.preposition_regex, middle_str, True
                        ):
                ...
                if followed_str and not has_before_date:
                    extracted_result = next(
                        iter(self.config.single_date_extractor.extract(followed_str, reference)), None
                    )
                    if extracted_result:
    ...
    def merge_two_time_points(
        self, source: str, reference: datetime, date_ers: [ExtractResult], time_ers: [ExtractResult]
    ) -> List[Token]:
        tokens = []
        ...
        while index < len(time_points) - 1:
            if (
                time_points[index].type == Constants.SYS_DATETIME_TIME
                and time_points[index + 1].type == Constants.SYS_DATETIME_TIME
            ):
                index += 1
                break
        ...
        while index < len(points) - 1:
            ...
            if mid_end - mid_begin > 0:
                ...
                if not mid_str.strip() or mid_str.strip().startswith(self.config.token_before_date):
                    ...
                    extended_str = (
                        points[index].text + source[int(points[index + 1].start + points[index + 1].length) :]
                    )
                    ...
                    tokens.append(
                        Token(points[index].start, offset + points[index + 1].start + points[index + 1].length)
                    )
                    index += 2
                    ...
                    if pause_match:
                        ...
                        if ending_match:
                            tokens.append(
                                Token(
                                    extracted_result.start,
                                    extracted_result.start + extracted_result.length + match.end(),
                                )
                            )
                    ...
                    if before_str:
                        ...
                        for er_time in ers_time:
                            ...
                            if not middle_str:
                                tokens.append(
                                    Token(
                                        er_time.start, er_time.start + er_time.length + len(middle_str) + token.length
                                    )
                                )
    ...
    def match_prefix_regex_in_segment(string: str, match: Match, in_prefix: bool):
        ...
        return match and substring.strip() != ''
...
class BaseDateTimePeriodParser(DateTimeParser):
    ...
    def is_before_or_after_mod(self, mod: str) -> bool:
        if not self.config.check_both_before_after:
            ...
            return mod and (
                ...
                or mod == TimeTypeConstants.AFTER_MOD
                or mod == TimeTypeConstants.UNTIL_MOD
                or mod == TimeTypeConstants.SINCE_MOD
            )
    ...
    def merge_date_and_time_periods(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        if len(extracted_results) == 0:
            ...
            if TimexUtil.is_range_timex(time_period_timex):
                date_result = self.config.date_extractor.extract(
                    trimmed_source.replace(extracted_results[0].text, ''), reference
                )
                ...
                if len(date_result) == 1 and date_text == date_result[0].text:
                    ...
                    if range_timex_components.is_valid:
                        begin_timex = TimexUtil.combine_date_and_time_timex(
                            date_timex, range_timex_components.begin_timex
                        )
                        end_timex = TimexUtil.combine_date_and_time_timex(date_timex, range_timex_components.end_timex)
                        result.timex = TimexUtil.generate_date_time_period_timex(
                            begin_timex, end_timex, range_timex_components.duration_timex
                        )

                        time_period_future_value = (
                            time_period_resolution_result.future_value.start,
                            time_period_resolution_result.future_value.end,
                        )
                        ...
                        if (
                            time_period_resolution_result.comment
                            and time_period_resolution_result.comment == Constants.COMMENT_AMPM
                        ):
                            ...
                            if (
                                begin_time.hour < Constants.HALF_DAY_HOUR_COUNT
                                and end_time.hour < Constants.HALF_DAY_HOUR_COUNT
                            ):
    ...
    def parse_simple_cases(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        extracted_result = next(
            iter(self.config.date_extractor.extract(source.replace(match.group(), ''), reference)), None
        )
    ...
    @staticmethod
    def get_two_points(
        begin_er: ExtractResult,
        end_er: ExtractResult,
        begin_parser: DateTimeParser,
    ...
    def merge_two_time_points(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        if both_have_dates:
            ...
            past_end = DateUtils.safe_create_from_min_value(
                past_begin.year, past_begin.month, past_begin.day, past_end.hour, past_end.minute, past_end.second
            )
            ...
            future_begin = DateUtils.safe_create_from_min_value(
                future_end.year,
                future_end.month,
                future_end.day,
                future_begin.hour,
            ...
            past_begin = DateUtils.safe_create_from_min_value(
                past_end.year, past_end.month, past_end.day, past_begin.hour, past_begin.minute, past_begin.second
            )
        ...
        if (
            am_pm_str_1
            and am_pm_str_1.endswith(Constants.COMMENT_AMPM)
            and am_pm_str_2
            and am_pm_str_2.endswith(Constants.COMMENT_AMPM)
        ):
            result.comment = Constants.COMMENT_AMPM
    ...
    def get_datetime(date: datetime, time: datetime) -> datetime:
        return DateUtils.safe_create_from_min_value(
    ...
    def parse_specific_time_of_day(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        if not match:
            match = self.config.am_desc_regex.search(trimmed_source)
            if not match:
            ...
            extracted_results = self.config.date_extractor.extract(
                (trimmed_before_str if trimmed_before_str != '' else before_str)
                + ' '
                + (trimmed_after_str if trimmed_after_str != '' else after_str),
                reference,
            )
            if len(extracted_results) == 0 or extracted_results[0].length < len(trimmed_before_str):
                ...
                if not valid:
                    ...
                    if len(extracted_results) == 0 or extracted_results[0].length != len(after_str):
                        if len(extracted_results) > 0 and extracted_results[0].start + extracted_results[
                            0
                        ].length == len(after_str):
            ...
            if len(time_period_extract_results) > 0:
                ...
                if time_parse_result:
                    period_future = (
                        time_parse_result.value.future_value.start,
                        time_parse_result.value.future_value.end,
                    )
            ...
            if not has_specific_time_period:
                ...
                format_str = '({}T{},{}T{},PT{}H)'
                result.timex = format_str.format(
                    parse_result.timex_str,
                    values.begin_hour,
                    parse_result.timex_str,
                    values.end_hour,
            ...
            result.future_value = (
                DateUtils.safe_create_from_min_value(
                    future_date.year, future_date.month, future_date.day, values.begin_hour, 0, 0
                ),
                DateUtils.safe_create_from_min_value(
                    future_date.year,
                    future_date.month,
            ...
            result.past_value = (
                DateUtils.safe_create_from_min_value(
                    past_date.year, past_date.month, past_date.day, values.begin_hour, 0, 0
                ),
                DateUtils.safe_create_from_min_value(
                    past_date.year, past_date.month, past_date.day, values.end_hour, values.end_min, values.end_min
                ),
            )
    ...
    def parse_duration(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        if len(extracted_result) == 1:
            ...
            if parse_result.value:
                ...
                if self.config.check_both_before_after and RegExpUtility.is_exact_match(
                    self.config.within_next_prefix_regex, after_str, True
                ):
                    end_time = begin_time + timedelta(seconds=swift_seconds)
                ...
                result.timex = (
                    f'({DateTimeFormatUtil.luis_date_from_datetime(begin_time)}T'
                    f'{DateTimeFormatUtil.luis_time_from_datetime(begin_time)},'
                    + f'{DateTimeFormatUtil.luis_date_from_datetime(end_time)}T'
                    f'{DateTimeFormatUtil.luis_time_from_datetime(end_time)},' + f'{duration_result.timex})'
                )
    ...
    def get_valid_connector_mod_for_date_and_time_period(self, source: str, in_prefix: bool):
        ...
        before_after_regex_tuple = [
            (self.config.before_regex, TimeTypeConstants.UNTIL_MOD, TimeTypeConstants.BEFORE_MOD),
            (self.config.after_regex, TimeTypeConstants.SINCE_MOD, TimeTypeConstants.AFTER_MOD),
        ]
        ...
        for regexp in before_after_regex_tuple:
            ...
            if match and match.success:
                mod = (
                    regexp[2] if in_prefix else (regexp[1] if RegExpUtility.get_group(match, 'include') else regexp[2])
                )
                return mod
    ...
    def parse_relative_unit(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        if unit_str == 'D':
            end_time = DateUtils.safe_create_from_min_value(
                begin_time.year, begin_time.month, begin_time.day
    ...
    def parse_date_with_period_prefix(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        if len(date_result) > 0:
            ...
            if not match and self.config.check_both_before_after:
                after_str = source[
                    date_result[-1].start
                    + date_result[-1].length : len(source)
                    - date_result[-1].start
                    + date_result[-1].length
                ]
        ...
        if date_extract_result and time_extract_result:
            ...
            if date_str_end < time_extract_result.start:
                ...
                if mod_str and self.config.check_both_before_after:
                    mod_str = (
                        self.get_valid_connector_mod_for_date_and_time_period(after_str, False)
                        if len(mid_str) <= 4
                        else None
                    )
