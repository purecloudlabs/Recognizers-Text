...
class FrenchDatePeriodParserConfiguration(DatePeriodParserConfiguration):
    ...
    def __init__(self, config: BaseDateParserConfiguration):
        self._relative_regex = RegExpUtility.get_safe_reg_exp(FrenchDateTime.RelativeRegex)
        ...
        self._next_suffix_regex = RegExpUtility.get_safe_reg_exp(FrenchDateTime.NextSuffixRegex)
        self._past_suffix_regex = RegExpUtility.get_safe_reg_exp(FrenchDateTime.PastSuffixRegex)
        ...
        self._complex_dateperiod_regex = RegExpUtility.get_safe_reg_exp(FrenchDateTime.ComplexDatePeriodRegex)
        self._relative_decade_regex = RegExpUtility.get_safe_reg_exp(FrenchDateTime.RelativeDecadeRegex)
        self._reference_date_period_regex = RegExpUtility.get_safe_reg_exp(FrenchDateTime.ReferenceDatePeriodRegex)
        self._less_than_regex = RegExpUtility.get_safe_reg_exp(FrenchDateTime.LessThanRegex)
    ...
    def get_swift_day_or_month(self, source: str) -> int:
        ...
        if (
            trimmed_source.endswith('dernière')
            or trimmed_source.endswith('dernières')
            or trimmed_source.endswith('derniere')
            or trimmed_source.endswith('dernieres')
        ):
            swift = -1
    ...
    def is_future(self, source: str) -> bool:
        trimmed_source = source.strip().lower()
        return any(trimmed_source.startswith(o) for o in FrenchDateTime.FutureStartTerms) or any(
            trimmed_source.endswith(o) for o in FrenchDateTime.FutureEndTerms
        )
    ...
    def is_week_only(self, source: str) -> bool:
        trimmed_source = source.strip().lower()
        return (
            any(trimmed_source.endswith(o) for o in FrenchDateTime.WeekTerms)
            or (
                any(trimmed_source.__contains__(o) for o in FrenchDateTime.WeekTerms)
                and (self._next_suffix_regex.search(trimmed_source) or self._past_suffix_regex.search(trimmed_source))
            )
        ) and not any(trimmed_source.endswith(o) for o in FrenchDateTime.WeekendTerms)
