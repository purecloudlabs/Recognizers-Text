...
from ...resources import CatalanDateTime
from .datetimeperiod_parser_config import CatalanDateTimePeriodParserConfiguration


class CatalanDateTimePeriodParser(BaseDateTimePeriodParser):
    def __init__(self, config: CatalanDateTimePeriodParserConfiguration):
    ...
    def parse_specific_time_of_day(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        start_index = len(CatalanDateTime.Tomorrow) if trimmed_text.startswith(
            CatalanDateTime.Tomorrow) else 0
        ...
        matches = list(RegExpUtility.get_safe_reg_exp(
            CatalanDateTime.TimeOfDayRegex).finditer(trimmed_text[start_index:]))
        if matches:
            match = matches[0]
