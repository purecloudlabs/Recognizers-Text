...
class SpanishCardinalExtractor(BaseNumberExtractor):
    ...
    @property
    def regexes(self) -> List[ReVal]:
        return (SpanishIntegerExtractor(self.placeholder).regexes +
                SpanishDoubleExtractor(self.placeholder).regexes)
...
class SpanishIntegerExtractor(BaseNumberExtractor):
    ...
    def regexes(self) -> List[ReVal]:
        return [
            ReVal(
                re=SpanishNumeric.NumbersWithPlaceHolder(self.placeholder),
                val='IntegerNum'),
            ReVal(
...
class SpanishDoubleExtractor(BaseNumberExtractor):
    ...
    def regexes(self) -> List[ReVal]:
        return [
            ReVal(
                re=SpanishNumeric.DoubleDecimalPointRegex(self.placeholder),
                val='DoubleNum'),
            ReVal(
                re=SpanishNumeric.DoubleWithoutIntegralRegex(self.placeholder),
                val='DoubleNum'),
            ReVal(
            ...
            ReVal(
                ...
                val='DoubleNum'),
            ReVal(
                re=self._generate_format_regex(LongFormatMode.DOUBLE_NO_BREAK_SPACE_COMMA,
                    self.placeholder),
                val='DoubleNum')
        ]
