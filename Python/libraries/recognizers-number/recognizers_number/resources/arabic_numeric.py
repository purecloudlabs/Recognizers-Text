...
class ArabicNumeric:
    ...
    NumbersWithDozenSuffix = (
        '(((?<!\\d+\\s*)([-]\\s*)?)|(?<=\\b))(\\d+\\s+)?(دستة|دستات|دست|دزينة|دزينات|دزينتين)(?=\\b)'
    )
    ...
    OrdinalNumericRegex = '(?<=\\b)(?:\\d{1,3}(\\s*,\\s*\\d{3})*\\s*th)(?=\\b)'
    OrdinalRoundNumberRegex = f'({RoundNumberOrdinalRegex})'
    OrdinalEnglishRegex = f'(?<=\\b){AllOrdinalRegex}(?=\\b)'
    FractionNotationWithSpacesRegex = '(((?<={?[\\u0600-\\u06ff]}|^)-\\s*)|(?<=\\b))\\d+\\s+\\d+[/]\\d+(?=(\\b[^/]|$))'
    FractionNotationWithSpacesRegex2 = (
        '(((?<={?[\\u0600-\\u06ff]}|^)-\\s*)|(?<![/-])(?<=\\b))\\d+[/]\\d+(?=(\\b[^/]|$))(\\s*\\d+)'
    )
    FractionNotationRegex = '(((?<={?[\\u0600-\\u06ff]}|^)-\\s*)|(?<![/-])(?<=\\b))\\d+[/]\\d+(?=(\\b[^/]|$))'
    FractionOrdinalPrefix = '(الوزن|المحتوى:?)'
    ...
    FractionMultiplierRegex = (
        f'(?<fracMultiplier>\\s+{ArabicAndRegex}\\s(أ|واحد|{TwoToNineIntegerRegex})\\s+({ArabicPartOfItem}))'
    )
    ...
    DoubleExponentialNotationRegex = '(((?<!\\d+\\s*)([-]\\s*)?)|((?<=\\b)(?<!\\d+[.,])))(\\d+(\\u202A?[.,]\\u202A?\\d+)?)e([+-]*[\\u0660-\\u0669]\\d*)(?=\\b)'
    DoubleCaretExponentialNotationRegex = '(((?<!\\d+\\s*)([-]\\s*)?)|((?<=\\b)(?<!\\d+[.,])))(\\d+(\\u202A?[.,]\\u202A?\\d+)?)[+-]*\\^([+-]*[\\u0660-\\u0669]([.,])?\\d*)(?=\\b)'
    ...
    ConnectorRegex = '(?<spacer>و)'
    TillRegex = '(الى|إلى|خلال|--|-|—|——|~|–)'
    MoreRegex = '(?:(اكثر|فوق|أكبر|أعظم|أطول|يتجاوز|تفوق|أعلى|أكثر)|(?<!<|=)>)'
    LessRegex = '(?:(أقل|اقل|اصغر|أصغر|أخفض|ادنى)(\\s*من)?|تحت|(?<!>|=)<)'
    EqualRegex = '(يساوي|(?<!<|>)=)'
    ...
    MoreOrEqualSuffix = '((أو|او)\\s+(((أكبر|أعظم|أطول|فوق|اكثر|اكثر|اكبر|أكثر)((?!\\s+من)|(\\s+من(?!(\\s*\\d+)))))|((فوق|أكبر|أطول|اكثر)(?!\\s+من))))'
    ...
    LessOrEqualSuffix = '((أ|ا)?و\\s+(أقل)((?!\\s+من)|(\\s+من(?!(\\s*\\d+)))))'
    NumberSplitMark = '(?![.،](?!\\d+))'
    MoreRegexNoNumberSucceed = (
        '((أكبر|أعظم|أطول|فوق|اكثر)((?!\\s+من)|\\s+(من(?!(\\s*\\d+))))|(فوق|أكبر|أعظم)(?!(\\s*\\d+)))'
    )
    LessRegexNoNumberSucceed = (
        '((أقل|أصغر)((?!\\s+من)|\\s+(من(?!(\\s*\\d+))))|(تحت|اقل|أقل|أصغر)(?!((\\s*\\d+)|\\s*من)))'
    )
    EqualRegexNoNumberSucceed = '((يساوي)(?!(\\s*\\d+)))'
    ...
    TwoNumberRangeRegex1 = (
        f'بين\\s*(ال)?(?<number1>({NumberSplitMark}.)+)\\s*و\\s*(ال)?(?<number2>({NumberSplitMark}.)+)'
    )
    ...
    AmbiguousFractionConnectorsRegex = '(\\bمن|بين|من|بين\\b)'
