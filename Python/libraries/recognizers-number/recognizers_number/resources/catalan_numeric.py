...
class CatalanNumeric:
    ...
    RoundNumberIntegerRegex = '((?:cent|miler|milion|milió|mil milions|bilió)s?|mil)'
    ZeroToNineIntegerRegex = '(?:tres|set|vuit|quatre|cinc|zero|nou|un|dos|sis)'
    TwoToNineIntegerRegex = '(?:tres|set|vuit|quatre|cinc|nou|dos|sis)'
    NegativeNumberTermsRegex = '(?<negTerm>(menys|negatiu)\\s+)'
    NegativeNumberSignRegex = f'^{NegativeNumberTermsRegex}.*'
    TenToNineteenIntegerRegex = '(?:disset|tretze|catorze|divuit anys|dinou|quinze|setze|onze|dotze|deu)'
    TensNumberIntegerRegex = '(?:setanta|vint|trenta|vuitanta|noranta|quaranta|cinquanta|seixanta)'
    ...
    NumbersWithDozenSuffix = '(((?<=\\W|^)-\\s*)|(?<=\\b))\\d+\\s+(dotzena)s?(?=\\b)'
    AllIntRegexWithLocks = f'((?<=\\b){AllIntRegex}(?=\\b))'
    RoundNumberOrdinalRegex = '(?:centèsim|mil·lèssim|milionèsima|mil milions|bilionsè)'
    NumberOrdinalRegex = '(?:primer|segon[a]?|tercer|quart|cinquè|cinquena|sisè|sisena|setè|setena|vuitè|vuitena|novè|desè|desena|onzè|dotzè|tretzè|catorzè|quinzena|setzè|dissetè|divuitè|dinou|vintè|thirtieth|trent[èe]|cinquantè|seixanta|setanta|vuitanta|noranta)'
    ...
    OrdinalSuffixRegex = '(?<=\\b)(?:(\\d*(1r|2n|3r|4t|[5-99][èe])))(?=\\b)'
