...
class DutchNumeric:
    ...
    OrdinalSuffixRegex = '(?<=\\b)((\\d+\\s*e)|[18]\\s*ste|[092-7]\\s*de|([0-9]*1[0-9]\\s*de)|([0-9]*[2-9][0-9]\\s*ste)|([0-9]*[0]([18]\\s*ste|[092-7]\\s*de)))(?=\\b)'
    OrdinalNumericRegex = '(?<=\\b)(\\d{1,3}(\\s*.\\s*\\d{3})*\\s*e)(?=\\b)'
    ...
    FractionUnitsRegex = '((?<onehalf>anderhalve|anderhalf)|(?<quarter>driekwart)|half|halve|helft|kwart)'
    FractionHalfRegex = '([eë]nhalf|[eë]nhalve|ëneenhal(f|ve))$'
    ...
    LessRegex = '((minder|lager|kleiner)(\\s+dan|\\s+als)?|beneden|onder|<)'
    EqualRegex = '(gelijk(\\s+(aan|tot|als|dan))?|(?<!<|>)=)'
