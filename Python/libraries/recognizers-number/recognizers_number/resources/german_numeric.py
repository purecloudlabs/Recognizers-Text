...
class GermanNumeric:
    ...
    ZeroToNineIntegerRegex = '(drei|sieben|acht|vier|fuenf|fünf|null|neun|eins|(ein(?!($|\\.|,|!|\\?)))|eine[rn]?|zwei|zwo|sechs)'
    ...
    AnIntRegex = '(eine?)(?=\\s)'
    TenToNineteenIntegerRegex = '(siebzehn|dreizehn|vierzehn|achtzehn|neunzehn|fünfzehn|fuenfzehn|sechzehn|elf|zwoelf|zwölf|zehn)'
    TensNumberIntegerRegex = '(siebzig|zwanzig|dreißig|achtzig|neunzig|vierzig|fuenfzig|fünfzig|sechzig|hundert|tausend)'
    ...
    NumbersWithDozenSuffix = '(((?<!\\d+\\s*)-\\s*)|(?<=\\b))\\d+\\s+dutzend(e)?(?=\\b)'
    ...
    RoundNumberOrdinalRegex = '(hundertst(er|es|en|el|e)?|tausendst(er|es|en|el|e)?|millionst(er|es|en|el|e)?|milliardst(er|es|en|el|e)?|billionst(er|es|en|el|e)?)'
    RelativeOrdinalRegex = '(?<relativeOrdinal>(nächste|vorherige|aktuelle|jetzige|(vor|dritt)?letzte)[nr]?|zuletzt|früher)'
    ...
    FractionUnitsRegex = '((?<onehalf>anderthalb|einundhalb)|(?<quarter>dreiviertel))'
    FractionHalfRegex = '(einhalb(es)?)$'
    ...
    ConnectorRegex = '(?<spacer>und)'
    TillRegex = '(bis(\\s+zu)?|--|-|—|——|~|–)'
    MoreRegex = '(?:(größer|höher|mehr)(\\s+als)?|über|darüber(hinaus)?|(?<!<|=)>)'
    LessRegex = '(?:(weniger|winziger|kleiner|wenig)(\\s+als)?|darunter|unter|(?<!>|=)<)'
    EqualRegex = '(gleich(\\s+(als|zu))?|(?<!<|>)=)'
    ...
    MoreOrEqualSuffix = '((und|oder)\\s+(((mehr|größer|höher)((?!\\s+als)|(\\s+als(?!(\\s*\\d+)))))|((über|darüber)(?!\\s+als))))'
    ...
    LessOrEqualSuffix = '((und|oder)\\s+(weniger|geringer|kleiner|winziger)((?!\\s+als)|(\\s+als(?!(\\s*\\d+)))))'
    NumberSplitMark = '(?![,.](?!\\d+))'
    MoreRegexNoNumberSucceed = '((größer|mehr|höhrer|breiter)((?!\\s+als)|\\s+(als(?!(\\s*\\d+))))|((dar)?über)(?!(\\s*\\d+)))'
    LessRegexNoNumberSucceed = '((kleiner|weniger|winziger)((?!\\s+als)|\\s+(als(?!(\\s*\\d+))))|((dar)?unter)(?!(\\s*\\d+)))'
    EqualRegexNoNumberSucceed = '(gleich(s|ing)?((?!\\s+(zu|als))|(\\s+(zu|als)(?!(\\s*\\d+)))))'
    ...
    HalfADozenRegex = 'ein\\s+halbes\\s+dutzend'
