...
class GermanNumeric:
    ...
    TensNumberIntegerRegex = (
        '(siebzig|zwanzig|dreißig|achtzig|neunzig|vierzig|fuenfzig|fünfzig|sechzig|hundert|tausend)'
    )
    ...
    RelativeOrdinalRegex = (
        '(?<relativeOrdinal>(nächste|vorherige|aktuelle|jetzige|(vor|dritt)?letzte)[nr]?|zuletzt|früher)'
    )
    ...
    MoreOrEqualSuffix = (
        '((und|oder)\\s+(((mehr|größer|höher)((?!\\s+als)|(\\s+als(?!(\\s*\\d+)))))|((über|darüber)(?!\\s+als))))'
    )
    ...
    MoreRegexNoNumberSucceed = (
        '((größer|mehr|höhrer|breiter)((?!\\s+als)|\\s+(als(?!(\\s*\\d+))))|((dar)?über)(?!(\\s*\\d+)))'
    )
    LessRegexNoNumberSucceed = (
        '((kleiner|weniger|winziger)((?!\\s+als)|\\s+(als(?!(\\s*\\d+))))|((dar)?unter)(?!(\\s*\\d+)))'
    )
    ...
    TwoNumberRangeRegex1 = (
        f'zwischen\\s*(der\\s+)?(?<number1>({NumberSplitMark}.)+)\\s*und\\s*(der\\s+)?(?<number2>({NumberSplitMark}.)+)'
    )
