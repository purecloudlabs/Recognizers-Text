...
class ItalianNumeric:
    ...
    RoundNumberIntegerRegex = '(cent(o|(?!\\b)|(?=\'))|mill(e|(?!\\b)|(?=\'))|mila|miliard([oi]|(?!\\b)|(?=\'))|(milion|bilion|trilion)([ei]|(?!\\b)|(?=\')))'
    ZeroToNineIntegerRegex = '(un[oa]?|due|tre|quattro|cinque|sei|sette|otto|nove|zero)'
    TwoToNineIntegerRegex = '(due|tre|quattro|cinque|sei|sette|otto|nove)'
    NegativeNumberTermsRegex = '(?<negTerm>meno\\s+)'
    NegativeNumberSignRegex = f'^{NegativeNumberTermsRegex}.*'
    AnIntRegex = '(un)(?=\\s)'
    TenToNineteenIntegerRegex = '(diciott(o|(?!\\b)|(?=\'))|(diciassett|diciannov)(e|(?!\\b)|(?=\'))|(tredic|quattordic|quindic|sedic|undic|dodic|diec)(i|(?!\\b)|(?=\')))'
    TensNumberIntegerRegex = (
        '(vent(i|(?!\\b)|(?=\'))|(settant|trent|ottant|novant|quarant|cinquant|sessant)(a|(?!\\b)|(?=\')))'
    )
    ...
    NumbersWithDozenSuffix = '(((?<!\\d+\\s*)-\\s*)|(?<=\\b))\\d+\\s+dozzin[ae](?=\\b)'
    ...
    RoundNumberOrdinalRegex = (
        '(centesim[oaie]|millesim[oaie]|milionesim[oaie]|miliardesim[oaie]|bilionesim[oaie]|trilionesim[oaie])'
    )
    OneToNineOrdinalRegex = (
        '(prim[oaie]|second[oaie]|terz[oaie]|quart[oaie]|quint[oaie]|sest[oaie]|settim[oaie]|ottav[oaie]|non[oaie])'
    )
    NumberOrdinalRegex = '(prim[oaie]|second[oaie]|terz[oaie]|quart[oaie]|quint[oaie]|sest[oaie]|settim[oaie]|ottav[oaie]|non[oaie]|decim[oaie]|undicesim[oaie]|dodicesim[oaie]|tredicesim[oaie]|quattordicesim[oaie]|quindicesim[oaie]|sedicesim[oaie]|diciassettesim[oaie]|diciottesim[oaie]|diciannovesim[oaie]|ventesim[oaie]|trentesim[oaie]|quarantesim[oaie]|cinquantesim[oaie]|sessantesim[oaie]|settantesim[oaie]|ottantesim[oaie]|novantesim[oaie])'
    OneToNineOrdinalCompoundRegex = '(un|du|tre|quattr|cinqu|sei|sett|ott|nov)esim[oaie]'
    RelativeOrdinalRegex = '(?<relativeOrdinal>precedent[ei]|seguent[ei]|prossim[aoei]|corrent[ei]|successiv[aoei]|penultim[oaei]|terzultim[oaei]|(l\')?(ultim[oaei]|attual[ei])|quell[ao]\\s+prima\\s+dell\'ultim[ao])'
    ...
    OrdinalSuffixRegex = '(?<=\\b)(\\d+(°|(esi)?m[oaie]))'
    OrdinalNumericRegex = '(?<=\\b)(\\d{1,3}(\\s*,\\s*\\d{3})*(°|(esi)?m[oaie]))'
    OrdinalRoundNumberRegex = f'(?<!(un)\\s+){RoundNumberOrdinalRegex}'
    OrdinalItalianRegex = f'(?<=\\b){AllOrdinalRegex}(?=\\b)'
    FractionNotationWithSpacesRegex = '(((?<=\\W|^)-\\s*)|(?<=\\b))\\d+\\s+(e\\s+)?\\d+[/]\\d+(?=(\\b[^/]|$))'
    ...
    RoundMultiplierWithFraction = (
        f'(?<multiplier>(?:milion[ei]|miliard[oi]|bilion[ei]|trillion[ei]))(?={FractionMultiplierRegex}?$)'
    )
    ...
    ConnectorRegex = '(?<spacer>e)'
    DoubleExponentialNotationRegex = '(((?<!\\d+\\s*)-\\s*)|((?<=\\b)(?<!\\d+,)))(\\d+(,\\d+)?)e([+-]*[1-9]\\d*)(?=\\b)'
    DoubleCaretExponentialNotationRegex = (
        '(((?<!\\d+\\s*)-\\s*)|((?<=\\b)(?<!\\d+,)))(\\d+(,\\d+)?)\\^([+-]*[1-9]\\d*)(?=\\b)'
    )
    TillRegex = '(a|fino\\s+a|--|-|—|——|~|–)'
    MoreRegex = '((più\\s+grand[ei]|più\\s+(in\\s+)?alt[oi]|maggior[ei]|al\\s+di\\s+sopra|più)(\\s+di)?|sopra(\\s+i)?|superior[ei](\\s+a)?|(?<!<|=)>)'
    LessRegex = '((meno|più\\s+(in\\s+)?bass[oi]|più\\s+piccol[oi]|minor[ei]|al\\s+di\\s+sotto)(\\s+di)?|sotto(\\s+i)?|inferior[ei](\\s+a)?|(?<!>|=)<)'
    EqualRegex = '(ugual[ei](\\s+a)?|(?<!<|>)=)'
    ...
    MoreOrEqualSuffix = '((e|o)\\s+(((più\\s+grand[ei]|più\\s+(in\\s+)?alt[oi]|maggior[ei]|al\\s+di\\s+sopra|più)((?!\\s+di)|(\\s+di(?!(\\s*\\d+)))))|((superior[ei])((?!\\s+a)|(\\s+a(?!(\\s*\\d+)))))))'
    ...
    LessOrEqualSuffix = '((e|o)\\s+(((meno|più\\s+(in\\s+)?bass[oi]|più\\s+piccol[oi]|minor[ei]|al\\s+di\\s+sotto)((?!\\s+di)|(\\s+di(?!(\\s*\\d+)))))|((meno|inferior[ei])((?!\\s+a)|(\\s+a(?!(\\s*\\d+)))))))'
    NumberSplitMark = '(?![,.](?!\\d+))'
    MoreRegexNoNumberSucceed = '((più\\s+grand[ei]|più\\s+(in\\s+)?alt[oi]|maggior[ei]|al\\s+di\\s+sopra|più)((?!\\s+di)|\\s+(di(?!(\\s*\\d+))))|(sopra(\\s+i)?|superior[ei](\\s+a)?)(?!(\\s*\\d+)))'
    LessRegexNoNumberSucceed = '((meno|più\\s+(in\\s+)?bass[oi]|più\\s+piccol[oi]|minor[ei]|al\\s+di\\s+sotto)((?!\\s+di)|\\s+(di(?!(\\s*\\d+))))|(sotto(\\s+i)?|inferior[ei](\\s+a)?)(?!(\\s*\\d+)))'
    EqualRegexNoNumberSucceed = '(ugual[ei]((?!\\s+a)|(\\s+a(?!(\\s*\\d+)))))'
    ...
    HalfADozenRegex = 'mezza\\s+dozzina'
    ...
    AmbiguousFractionConnectorsRegex = '(\\bnel\\b)'
