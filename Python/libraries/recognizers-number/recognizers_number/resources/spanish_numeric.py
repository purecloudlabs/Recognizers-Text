...
class SpanishNumeric:
    ...
    HundredsNumberIntegerRegex = '(cuatrocient[ao]s|trescient[ao]s|seiscient[ao]s|setecient[ao]s|ochocient[ao]s|novecient[ao]s|doscient[ao]s|quinient[ao]s|(?<!por\\s+)(cien(to)?))'
    RoundNumberIntegerSingRegex = '(((mil\\s+)?mi|bi|cuatri|quinti|sexti|septi)ll[oó]n|mil)'
    RoundNumberIntegerRegex = f'({RoundNumberIntegerSingRegex}(es)?)'
    ZeroToNineIntegerRegex = '(cuatro|cinco|siete|nueve|cero|tres|seis|ocho|dos|un[ao]?)'
    TwoToNineIntegerRegex = '(cuatro|cinco|siete|nueve|tres|seis|ocho|dos)'
    TenToNineteenIntegerRegex = '(diecisiete|diecinueve|diecis[eé]is|dieciocho|catorce|quince|trece|diez|once|doce)'
    TwentiesIntegerRegex = (
        '(veinti(cuatro|cinco|siete|nueve|tr[eé]s|s[eé]is|ocho|d[oó]s|[uú]n[oa]?)|ventiun[ao]|veinte)'
    )
    TensNumberIntegerRegex = '(cincuenta|cuarenta|treinta|se[st]enta|ochenta|noventa)'
    NegativeNumberTermsRegex = '(?<negTerm>(?<!(al|lo)\\s+)menos\\s+)'
    ...
    NumbersWithDozenSuffix = '(((?<=\\W|^)-\\s*)|(?<=\\b))\\d+\\s+(docena|dz|doz)s?(?=\\b)'
    AllIntRegexWithLocks = f'((?<=\\b){AllIntRegex}(?=\\b))'
    AllIntRegexWithDozenSuffixLocks = (
        f'(?<=\\b)(((media\\s+)?\\s+docena)|({AllIntRegex}\\s+(y|con)\\s+)?({AllIntRegex}\\s+(docena|dz|doz)s?))(?=\\b)'
    )
    SimpleRoundOrdinalRegex = '(mil[eé]simo|millon[eé]sim[oa]|billon[eé]sim[oa]|trillon[eé]sim[oa]|cuatrillon[eé]sim[oa]|quintillon[eé]sim[oa]|sextillon[eé]sim[oa]|septillon[eé]sim[oa])'
    OneToNineOrdinalRegex = (
        '(primer[oa]?|segund[oa]|tercer[oa]?|cuart[oa]|quint[oa]|sext[oa]|s[eé]ptim[oa]|octav[oa]|noven[oa])'
    )
    TensOrdinalRegex = '(nonag[eé]sim[oa]|octog[eé]sim[oa]|septuag[eé]sim[oa]|sexag[eé]sim[oa]|quincuag[eé]sim[oa]|cuadrag[eé]sim[oa]|trig[eé]sim[oa]|vig[eé]sim[oa]|d[eé]cim[oa])'
    HundredOrdinalRegex = '(cent[eé]sim[oa]|ducent[eé]sim[oa]|tricent[eé]sim[oa]|cuadringent[eé]sim[oa]|quingent[eé]sim[oa]|sexcent[eé]sim[oa]|septingent[eé]sim[oa]|octingent[eé]sim[oa]|noningent[eé]sim[oa])'
    SpecialUnderHundredOrdinalRegex = '(und[eé]cim[oa]|duod[eé]cim[oa]|decimoctav[oa])'
    UnderHundredOrdinalRegex = (
        f'({SpecialUnderHundredOrdinalRegex}|(({TensOrdinalRegex}(\\s)?)?{OneToNineOrdinalRegex})|{TensOrdinalRegex})'
    )
    ...
    RelativeOrdinalRegex = '(?<relativeOrdinal>(antes\\s+de|anterior\\s+a)(l|\\s+la)\\s+[uú]ltim[ao]|((ante)?pen)?[uú]ltim[ao]s?|pr[oó]xim[ao]s?|anterior(es)?|actual(es)?|siguientes?)'
    ...
    OrdinalSuffixRegex = '(?<=\\b)(\\d*((1(er|r[oa])|2d[oa]|3r[oa]|4t[oa]|5t[oa]|6t[oa]|7m[oa]|8v[oa]|9n[oa]|0m[oa]|11[vm][oa]|12[vm][oa])|\\d\\.?[ºª]))(?=\\b)'
    ...
    TillRegex = '(\\ba\\b|hasta|--|-|—|——|~|–)'
    MoreRegex = '(más\\s+(alt[oa]s?|grandes)\\s+que|(m[áa]s|mayor(es)?|superior(es)?|por\\s+encima)\\b((\\s+(que|del?|al?))|(?=\\s+o\\b))|(?<!<|=)>)'
    LessRegex = '((meno(s|r(es)?)|inferior(es)?|por\\s+debajo)((\\s+(que|del?|al?)|(?=\\s+o\\b)))|más\\s+baj[oa]\\s+que|(?<!>|=)<)'
    EqualRegex = '((igual(es)?|equivalente(s)?|equivalen?)(\\s+(al?|que|del?))?|(?<!<|>)=)'
    ...
    MoreOrEqualSuffix = '((\\b(y|o)\\b\\s+(m[áa]s|mayor(es)?|superior(es)?)((?!\\s+(alt[oa]|baj[oa]|que|del?|al?))|(\\s+(que|del?|al?)(?!(\\s*\\d+)))))|como\\s+m[íi]nimo|por\\s+lo\\s+menos|al\\s+menos)\\b'
    ...
    LessOrEqualSuffix = '((\\b(y|o)\\b\\s+(meno(s|r(es)?|inferior(es)?))((?!\\s+(alt[oa]|baj[oa]|que|del?|al?))|(\\s+(que|del?|al?)(?!(\\s*\\d+)))))|como\\s+m[áa]ximo)\\b'
    NumberSplitMark = (
        f'(?![,.](?!\\d+))(?!\\s*\\b(((y|e)\\s+)?({LessRegex}|{MoreRegex}|{EqualRegex}|no|de)|pero|o|a)\\b)'
    )
    MoreRegexNoNumberSucceed = '(\\b(m[áa]s|mayor(es)?|superior(es)?)((?!\\s+(que|del?|al?))|\\s+((que|del?)(?!(\\s*\\d+))))|(por encima)(?!(\\s*\\d+)))\\b'
    LessRegexNoNumberSucceed = '(\\b(meno(s|r(es)?)|inferior(es)?)((?!\\s+(que|del?|al?))|\\s+((que|del?|al?)(?!(\\s*\\d+))))|(por debajo)(?!(\\s*\\d+)))\\b'
    EqualRegexNoNumberSucceed = (
        '(\\b(igual(es)?|equivalentes?|equivalen?)((?!\\s+(al?|que|del?))|(\\s+(al?|que|del?)(?!(\\s*\\d+)))))\\b'
    )
    ...
    AmbiguousFractionConnectorsRegex = '(\\b(en|de)\\b)'
    ...
    HalfADozenRegex = 'media\\s+docena'
    ...
    SuffixOrdinalMap = dict([("milesimo", 1000), ("millonesimo", 1000000), ("billonesimo", 1000000000000)])
    RoundNumberMap = dict(
        [
            ("mil", 1000),
            ("milesimo", 1000),
