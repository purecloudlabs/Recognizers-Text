...
class EnglishNumeric:
    ...
    NonStandardSeparatorVariants = [r'en-na', r'en-zw']  # South African English and other variants use comma as decimal separator
    ...
    NegativeNumberTermsRegex = '(?<negTerm>(minus|negative)\\s+)'
    NegativeNumberSignRegex = f'^{NegativeNumberTermsRegex}.*'
    AnIntRegex = '(an?)(?=\\s)'
    TenToNineteenIntegerRegex = '(?:seventeen|thirteen|fourteen|eighteen|nineteen|fifteen|sixteen|eleven|twelve|ten)'
    TensNumberIntegerRegex = '(?:seventy|twenty|thirty|eighty|ninety|forty|fifty|sixty)'
    ...
    NumberOrdinalRegex = '(?:first|second|third|fourth|fifth|sixth|seventh|eighth|nine?th|tenth|eleventh|twelfth|thirteenth|fourteenth|fifteenth|sixteenth|seventeenth|eighteenth|nineteenth|twentieth|thirtieth|fortieth|fiftieth|sixtieth|seventieth|eightieth|ninetieth)'
    RelativeOrdinalRegex = '(?<relativeOrdinal>(next|previous|current)\\s+one|(the\\s+second|next)\\s+to\\s+last|the\\s+one\\s+before\\s+the\\s+last(\\s+one)?|the\\s+last\\s+but\\s+one|(ante)?penultimate|last|next|previous|current)'
    ...
    OrdinalSuffixRegex = '(?<=\\b)(?:(\\d*(1st|2nd|3rd|[4-90]th))|(1[1-2]th))(?=\\b)'
    ...
    EqualRegex = '(equal(s|ing)?(\\s+(to|than))?|(?<!<|>)=)'
    ...
    MoreOrEqualSuffix = '((and|or)\\s+(((more|greater|higher|larger|bigger)((?!\\s+than)|(\\s+than(?!((\\s+or\\s+equal\\s+to)?\\s*\\d+)))))|((over|above)(?!\\s+than))))'
