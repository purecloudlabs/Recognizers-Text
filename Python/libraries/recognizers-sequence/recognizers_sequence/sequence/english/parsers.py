...
class PhoneNumberParser(SequenceParser):
    ...
    def score_phone_number(self, phone_number_text) -> float:
        ...
        score += (
            self.country_code_award
            if country_code_regex.search(phone_number_text)
            else self.area_code_award if area_code_regex.search(phone_number_text) else 0
        )
        ...
        if format_indicator_regex.search(phone_number_text):
            ...
            score += min(format_indicator_count, self.max_format_indicator_num) * self.formatted_award
            score -= (
                self.continue_format_indicator_deduction_score
                if any(len(match[0]) > 1 for match in format_matches)
                else 0
            )
            if self.single_bracket_regex.search(phone_number_text) and not self.complete_bracket_regex.search(
                phone_number_text
            ):
        ...
        if self.digit_regex.search(phone_number_text):
            score += (
                min(
                    (len(list(self.digit_regex.finditer(phone_number_text))) - self.phone_number_length_base),
                    self.max_length_award_num,
                )
                * self.length_award
            )
        ...
        if self.tail_same_digit_regex.search(phone_number_text):
            score -= (
                len(self.tail_same_digit_regex.search(phone_number_text)[0]) - self.tail_same_limit
            ) * self.tail_same_deduction_score

        # Pure digit deduction
        if self.pure_digit_regex.search(phone_number_text):
            score -= (
                (len(phone_number_text) - self.pure_digit_length_limit) * self.length_award
                if len(phone_number_text) > self.pure_digit_length_limit
                else 0
            )
        ...
        if self.continue_digit_regex.search(phone_number_text):
            score -= (
                max(len(list(self.continue_digit_regex.finditer(phone_number_text))) - 1, 0)
                * self.continue_digit_deduction_score
            )
