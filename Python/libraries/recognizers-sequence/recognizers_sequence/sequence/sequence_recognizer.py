...
class SequenceRecognizer(Recognizer[SequenceOptions]):
    def __init__(
        ...
        options: SequenceOptions = SequenceOptions.NONE,
    ...
    def initialize_configuration(self):
        self.register_model(
            'PhoneNumberModel',
            Culture.English,
            lambda options: PhoneNumberModel(
                PhoneNumberParser(), BasePhoneNumberExtractor(EnglishPhoneNumberExtractorConfiguration())
            ),
        )
        ...
        self.register_model(
            ...
            Culture.Chinese,
            lambda options: PhoneNumberModel(
                PhoneNumberParser(), BasePhoneNumberExtractor(ChinesePhoneNumberExtractorConfiguration())
            ),
        )
        ...
        self.register_model(
            ...
            Culture.Portuguese,
            lambda options: PhoneNumberModel(
                PhoneNumberParser(), BasePhoneNumberExtractor(PortuguesePhoneNumberExtractorConfiguration())
            ),
        )

        self.register_model(
            'EmailModel', Culture.English, lambda options: EmailModel(EmailParser(), EnglishEmailExtractor())
        )

        self.register_model(
            'IpAddressModel',
            Culture.English,
            lambda options: IpAddressModel(IpParser(), BaseIpExtractor(EnglishIpExtractorConfiguration(options))),
        )

        self.register_model(
            'IpAddressModel',
            Culture.Chinese,
            lambda options: IpAddressModel(IpParser(), BaseIpExtractor(ChineseIpExtractorConfiguration(options))),
        )

        self.register_model(
            'MentionModel', Culture.English, lambda options: MentionModel(MentionParser(), EnglishMentionExtractor())
        )

        self.register_model(
            'HashtagModel', Culture.English, lambda options: HashtagModel(HashtagParser(), EnglishHashtagExtractor())
        )

        self.register_model(
            'URLModel',
            Culture.English,
            lambda options: URLModel(URLParser(), BaseURLExtractor(EnglishURLExtractorConfiguration(options))),
        )

        self.register_model(
            'URLModel',
            Culture.Chinese,
            lambda options: URLModel(URLParser(), BaseURLExtractor(ChineseURLExtractorConfiguration(options))),
        )

        self.register_model(
            'GUIDModel', Culture.English, lambda options: GUIDModel(GUIDParser(), EnglishGUIDExtractor())
        )
...
def recognize_phone_number(
    query: str, culture: str, options: SequenceOptions = SequenceOptions.NONE, fallback_to_default_culture: bool = True
) -> List[ModelResult]:
